"""Unit tests for the clarity package."""
...
from app.tools.packages import AdjustClarityPackage, OperationContext


class AdjustClarityPackageTest(unittest.TestCase):
    """Validate clarity adjustment behavior."""

    def setUp(self) -> None:
        self.tmpdir = tempfile.TemporaryDirectory()
        self.package = AdjustClarityPackage()
        ...
        detail = Image.new("RGB", (16, 16), (120, 120, 120))
        for x in range(16):
            for y in range(16):
                if (x + y) % 2 == 0:
                    detail.putpixel((x, y), (145, 145, 145))
    ...
    def test_normalize_returns_clarity_payload(self) -> None:
        normalized = self.package.normalize(
            {"op": "adjust_clarity", "params": {"amount": 0.5}},
            OperationContext(image_path=self.detail_path),
        )
        self.assertAlmostEqual(normalized["amount"], 0.5)
        self.assertAlmostEqual(normalized["radius_scale"], 1.0)

    def test_normalize_accepts_legacy_top_level_strength_for_amount_packages(self) -> None:
        normalized = self.package.normalize(
            {"op": "adjust_clarity", "strength": 0.5, "params": {}},
            OperationContext(image_path=self.detail_path),
        )
        self.assertAlmostEqual(normalized["amount"], 0.5)

    def test_execute_increases_local_contrast(self) -> None:
        ...
        result = self.package.execute(
            {
                "op": "adjust_clarity",
                "region": "main_subject",
                "params": {"amount": 0.65, "feather_radius": 0},
            },
    ...
    def test_execute_runs_globally_when_mask_missing(self) -> None:
        result = self.package.execute(
            {"op": "adjust_clarity", "region": "main_subject", "params": {"amount": 0.65}},
