...
1. `deterministic`
   适合曝光、对比度、白平衡、局部提亮、去噪、锐化、裁剪、背景虚化。
2. `generative`
...
1. 把局部分析、确定性处理、生成式处理串起来。
2. 统一输入输出格式。
3. 保证不会破坏主图结构。

建议后做的原因：

1. Hybrid 依赖前面两类执行器都先稳定。
...
5. 再补 `build_plan`
6. 再补 `route_executor`
7. 再补 `execute_deterministic`
8. 再补 `evaluate_result`
9. 再补 `memory/retriever.py`
10. 再补 `update_memory`
11. 再补 `execute_generative`
12. 再补 `execute_hybrid`
13. 再补 `human_review`
14. 最后再接 API

一句话总结：

先把 LangGraph 主图做成稳定的“理解 -> 规划 -> 路由 -> 执行 -> 评估 -> 记忆”骨架，再逐步把每个节点内部从占位实现替换成真实能力。
